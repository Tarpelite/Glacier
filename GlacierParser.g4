...
ty_decl
    ...
    'type' type_hs '=' ktypedoc
    // type family declarations
    | 'type' 'family' type_hs opt_tyfam_kind_sig? opt_injective_info? where_type_family?
    ...
    // data/newtype family
    | 'data' 'family' type_hs opt_datafam_kind_sig?
    ;
...
ty_fam_inst_eqn_list
    :
    (open_hs ty_fam_inst_eqns? close)
    | ('{' '..' '}')
    | (open_hs '..' close)
    ;
...
at_decl_cls
    :
    ('data' 'family'? type_hs opt_datafam_kind_sig?)
    | ('type' 'family'? type_hs opt_at_kind_inj_sig?)
    | ('type' 'instance'? ty_fam_inst_eqn)
    ;
...
tycl_hdr
    :
    (tycl_context '=>' type_hs)
    | type_hs
    ;

tycl_hdr_inst
    :
    ('forall' tv_bndrs? '.' tycl_context '=>' type_hs)
    | ('forall' tv_bndrs? '.' type_hs)
    | (tycl_context '=>' type_hs)
    | type_hs
    ;
...
ctype
    ...
    | btype '=>' ctype
    | var '::' type_hs // not sure about this rule
    | type_hs
    ;
...
gadt_constrlist
    :
    'where' open_hs gadt_constrs? semi* close
    ;
